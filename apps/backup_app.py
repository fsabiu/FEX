        ...
        if websocket.open:
            ...
            for obj in recent_objects_list:
                if obj["tagName"] == "drone":
                    obj["lon"] = obj["lon_dron"]
                    obj["lat"] = obj["lat_dron"]
...
def pixel_to_gps_old(lat_dron, lon_dron, h_dron, pitch, yaw, roll, f, img_width, img_height, x_pixel, y_pixel):
...
def consumer(queue_in, pixel_mask, backend_queue, fps, camera_f, to_print = False, confidence_filters = None):
    ...
    try:
        ...
        last_drone_lat = 21.948178672207602
        last_drone_lon = 50.734347105368219
        ...
        while True:
            ...
            if frames >= init_frame:
                ...
                # Adding drone to object list
                drone_object = dict(tagName="drone", name="drone", lat_dron=last_drone_lat, lon_dron=last_drone_lon, id=0)
                for prop in ["lat_dron", "lon_dron", "h_dron"]:
                    if drone_dict is not None and prop in list(drone_dict.keys()):
                        last_drone_lat = drone_dict["lat_dron"]
                        last_drone_lon = drone_dict["lon_dron"]
                        drone_object[prop] = drone_dict[prop]
